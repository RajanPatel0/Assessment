...
const AdminSignIn = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await login(formData, '/adminDashboard');
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a1a] text-white font-sans flex items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-white/5 backdrop-blur-xl border border-white/10 p-8 rounded-3xl shadow-2xl relative z-10">
        ...
        <p className="text-slate-400 mb-8 text-sm">Enter your credentials to access your Admin dashboard.</p>
        ...
        <p className="text-center text-slate-400 mt-8 text-sm">
          Don't have an account? <a href="/adminSignUp" className="text-indigo-400 hover:text-indigo-300 font-medium">Create one</a>
        </p>
      </div>
    </div>
  );
