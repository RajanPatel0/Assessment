    ...
    <div className="min-h-screen bg-[#0a0a1a] text-white font-sans flex items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-white/5 backdrop-blur-xl border border-white/10 p-8 rounded-3xl shadow-2xl relative z-10">
        ...
        <div className="flex items-center gap-2 group cursor-pointer mb-10 w-fit" onClick={() => navigate('/')}>
          ...
          <span className="text-xl font-bold tracking-tight">TaskManager Admin</span>
        </div>

        <h2 className="text-3xl font-bold mb-2">Create Account</h2>
        <p className="text-slate-400 mb-8 text-sm">Join TaskManager and start organizing today.</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Admin Secret Key</label>
            <input 
              type="password" 
              name="adminSecret"
        ...
        <p className="text-center text-slate-400 mt-8 text-sm">
          Already have an account? <a href="/adminSignIn" className="text-indigo-400 hover:text-indigo-300 font-medium">Log in</a>
        </p>
      </div>
    </div>
  );
